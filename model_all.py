...
class VDS_Proposed(Dataset):
    ...
    def fft_gray(self, img, filter_rate):
        uv = torch.fft.fft2(img)
        shift_uv = torch.fft.fftshift(uv)
        h, w = img.shape
        ch, cw = int(h/2), int(w/2)
        rh, rw = int(filter_rate*ch), int(filter_rate*cw)
        shift_uv[ch-rh:ch+rh, cw-rw:cw+rw] = 0
        unshift_uv = torch.fft.fftshift(shift_uv)
        out_img = torch.fft.ifft2(unshift_uv).real
        return out_img

    def fft_rgb(self, img, filter_rate):
        c, h, w = img.shape
        out = torch.empty_like(img)
        for channel in range(c):
            out[channel, :, :] = self.fft_gray(img[channel, :, :], filter_rate)
        return out
    ...
    def __getitem__(self, idx):
        ...
        fft_img = self.fft_rgb(img_base, self.filter_rate)
        fft_img_tensor = self.img_transform(fft_img/255.)
        ...
        norm_idx = float(idx) / len(self.video)
        sample = {'img': tensor_image, 'idx': idx, 'norm_idx': norm_idx, 'fft': fft_img_tensor, 'lr': tensor_lr, 'resize': tensor_resize}
...
class HNeRV(nn.Module):
    def __init__(self, args):
        ...
        _, ks_lrdec1, ks_lrdec2 = [int(x) for x in args.lr_ks.split('_')]
        ...
        lr_decoder_layers1 = []
        self.ngf_lr1 = args.lr_fc_dim1
        ngf = args.lr_fc_dim1
        stem1 = MLP(dim_list=[args.fc_pe_1, int(self.ngf_lr1*args.fc_lr)], act=args.act)
        lr_decoder_layers1.append(stem1)
        for i, strd in enumerate(args.lr_dec_strds):
            reduction = args.reduce
            new_ngf = int(round(ngf / reduction))
            cur_blk = NeRVBlock(dec_block=True, conv_type=args.conv_type[1], ngf=ngf, new_ngf=new_ngf, 
                ks=min(ks_lrdec1+2*i, ks_lrdec2), strd=strd, bias=True, norm=args.norm, act=args.act)
            lr_decoder_layers1.append(cur_blk)
            ngf = new_ngf
        self.lr_decoder1 = nn.ModuleList(lr_decoder_layers1)
        self.lr_head_layer1 = nn.Conv2d(ngf, 3, 3, 1, 1)

        lr_decoder_layers2 = []
        self.ngf_lr2 = args.lr_fc_dim2
        ngf = args.lr_fc_dim2
        stem2 = MLP(dim_list=[args.fc_pe_2, int(self.ngf_lr2*args.fc_lr)], act=args.act)
        lr_decoder_layers2.append(stem2)
        for i, strd in enumerate(args.lr_dec_strds):
            reduction = args.reduce
            new_ngf = int(round(ngf / reduction))
            cur_blk = NeRVBlock(dec_block=True, conv_type=args.conv_type[1], ngf=ngf, new_ngf=new_ngf,  
                ks=min(ks_lrdec1+2*i, ks_lrdec2), strd=strd, bias=True, norm=args.norm, act=args.act)
            lr_decoder_layers2.append(cur_blk)
            ngf = new_ngf
        self.lr_decoder2 = nn.ModuleList(lr_decoder_layers2)
        self.lr_head_layer2 = nn.Conv2d(ngf, 3, 3, 1, 1)

    def forward(self, input, input_embed=None, encode_only=False, lr_data=None, embed_input=None):
        if input_embed != None:
        ...
        if lr_data != None:
            ...
            img_out = residue + lr_data
        elif embed_input != None:
            embed_input1 = embed_input[:, :20]
            embed_input2 = embed_input[:, 20:]

            lr_out1 = self.lr_decoder1[0](embed_input1)
            lr_out1 = lr_out1.view(lr_out1.size(0), self.ngf_lr1, 5, 10)
            for layer in self.lr_decoder1[1:]:
                lr_out1 = layer(lr_out1)
            lr_out1 = self.lr_head_layer1(lr_out1)

            lr_out2 = self.lr_decoder2[0](embed_input2)
            lr_out2 = lr_out2.view(lr_out2.size(0), self.ngf_lr2, 5, 10)
            for layer in self.lr_decoder2[1:]:
                lr_out2 = layer(lr_out2)
            lr_out2 = self.lr_head_layer2(lr_out2)

            out1 = lr_out1 + lr_out2
            out1 = OutImg(out1, self.out_bias, flag=False)

            residue = OutImg(self.head_layer(output), self.out_bias, flag=True)
            img_out = residue + out1
        else:
            residue = OutImg(self.head_layer(output), self.out_bias)
            img_out = residue
        ...
        return  img_out, residue, out1, lr_out1, lr_out2, embed_list, dec_time
