...
class VideoDataSet_HighPassFliter(Dataset):
    def __init__(self, args):
        self.video = [os.path.join(args.data_path, x) for x in sorted(os.listdir(args.data_path))]
        # Resize the input video and center crop
        self.crop_list, self.resize_list = args.crop_list, args.resize_list
    ...
    def sobel_filter(self, img):
        sobel_filter_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).float()
        sobel_filter_y = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).float()
        gradient_x = F.conv2d(img.unsqueeze(0), sobel_filter_x.unsqueeze(0).unsqueeze(0), padding=1)
        gradient_y = F.conv2d(img.unsqueeze(0), sobel_filter_y.unsqueeze(0).unsqueeze(0), padding=1)
        gradient_magnitude = torch.sqrt(gradient_x ** 2 + gradient_y ** 2)
        gradient_magnitude = gradient_magnitude.squeeze(0)
        return gradient_magnitude

    def sobel_filter_rgb(self, img):
        out = torch.empty_like(img)
        for c in range(img.shape[0]):
            out[c, :, :] = self.sobel_filter(img[c, :, :].float())
        return out
    ...
    def __getitem__(self, idx):
        ...
        norm_idx = float(idx) / len(self.video)
        sample = {'img': tensor_image, 'fft': fft_img, 'idx': idx, 'norm_idx': norm_idx}
...
class HNeRV(nn.Module):
    ...
    def forward(self, input, input_b, input_f, input_embed=None, encode_only=False, hnerv=False, scale=0.2):
        if input_embed != None:
            ...
            if hnerv:
                ...
                img_embed = self.encoder(input)
                img_b = self.encoder(input_b)
                img_f = self.encoder(input_f)
                diff = img_f - img_b
                diff = diff.abs()
                diff_max = diff.max()
                if diff_max == 0:
                ...
                diff_norm = (diff_norm * scale) + (1.0 - scale)
                img_embed = diff_norm * img_embed
